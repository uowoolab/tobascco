...
class Structure(object):
    ...
    def compute_inter_sbu_bonding(self, sbu1, cp1_id, sbu2, cp2_id):
        ...
        for atom in base_atoms:
            if bond_atoms:
                ...
                self.bonds.update({tuple(sorted((atom.index, bond_atom.index))): "S"})
    ...
    def _compute_bond_info(self):
        ...
        for (at1, at2), val in self.bonds.items():
            ...
            sym = (
                "."
                if all([i == 0 for i in supercells[image]])
                else "1_%i%i%i"
                % (tuple(np.array(supercells[image], dtype=int) + unit_repr))
            )
    ...
    def compute_overlap(self):
        ...
        for id, atom in enumerate(self.atoms):
            ...
            non_bonded = [
                i.index
                for i in self.atoms[id:]
                if tuple(sorted((atom.index, i.index))) not in self.bonds.keys()
            ]
            bonded = [
                i.index
                for i in self.atoms[id:]
                if tuple(sorted((atom.index, i.index))) in self.bonds.keys()
            ]
            ...
            for (atom1, atom2), dist in np.ndenumerate(dist_mat):
                ...
                if (
                    (id != id2)
                    and (id2 in non_bonded)
                    and (Radii[elem1] + Radii[elem2]) * self.options.overlap_tolerance
                    > dist
                ):
                    return True
                elif (
                    (id != id2)
                    and (id2 in bonded)
                    and 1.0 * self.options.overlap_tolerance > dist
                ):
                    return True
        return False
    ...
    def cif(self, c):
        ...
        c.add_data(
            "data",
            _audit_creation_method=CIF.label("TopCryst v.%s" % (self.options.version)),
        )
        if self.charge:
            c.add_data(
                "data",
                _chemical_properties_physical="net charge is %12.5f" % (self.charge),
            )
        ...
        for id, atom in enumerate(self.atoms):
            ...
            c.add_data(
                "atoms", _atom_site_type_symbol=CIF.atom_site_type_symbol(atom.element)
            )
            c.add_data(
                "atoms",
                _atom_site_description=CIF.atom_site_description(atom.force_field_type),
            )
            ...
            if self.options.find_symmetric_h:
                if atom.element == "H":
                    ...
                    symconst = -1
                c.add_data(
                    "atoms", _atom_site_constraints=CIF.atom_site_constraints(symconst)
                )
            fc = atom.scaled_pos(self.cell.inverse)
            c.add_data("atoms", _atom_site_fract_x=CIF.atom_site_fract_x(fc[0]))
            c.add_data("atoms", _atom_site_fract_y=CIF.atom_site_fract_y(fc[1]))
        ...
        for (at1, at2), (btype, dist, sym) in self.bonds.items():
            ...
            c.add_data("bonds", _geom_bond_distance=CIF.geom_bond_distance(dist))
            c.add_data(
...
class Symmetry(object):
    ...
    def add_structure(self, structure):
        ...
        self._scaled_coords = np.array(
            [atom.in_cell_scaled(self._inv_latt) for atom in structure.atoms]
        )
        self._angle_tol = -1.0
        self._element_symbols = [atom.element for atom in structure.atoms]
        self._numbers = np.array(
            [ATOMIC_NUMBER.index(i) for i in self._element_symbols]
        )
    ...
    def refine_cell(self):
        ...
        if dataset["number"] == 0:
            ...
            self.dataset["transformation_matrix"] = np.array(
            ...
            self._element_symbols = [
                ATOMIC_NUMBER[i] for i in ref_numbers[:num_atom_bravais]
            ]
    ...
    def get_space_group_operations(self):
        return [
            self.convert_to_string((r, t))
            for r, t in zip(self.dataset["rotations"], self.dataset["translations"])
        ]
    ...
    def get_equivalent_hydrogens(self):
        ...
        h_id = list(
            set(
                [i for id, i in enumerate(at_equiv) if self._element_symbols[id] == "H"]
            )
        )
