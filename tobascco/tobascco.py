...
class JobHandler(object):
    ...
    def _mpi_split(self):
        ...
        choice = [
            ...
            len(self.options.organic_sbus),
            len(self.options.metal_sbus),
        ]
        id = choice.index(
            max(choice)
        )  # just returns the first entry, if more than one has the same value..
    ...
    def direct_job(self):
        ...
        if not self.options.topologies:
            ...
            debug(
                "No topologies requested, trying all of the ones in the SBU database files."
        ...
        if not self.options.organic_sbus:
            self.options.organic_sbus = [
                sbu.identifier for sbu in self.sbu_pool if not sbu.is_metal
            ]
        # failsafe in case no metal sbus requested in the input file.
        if not self.options.metal_sbus:
            self.options.metal_sbus = [
                sbu.identifier for sbu in self.sbu_pool if sbu.is_metal
            ]
    ...
    def _build_structures_from_top(self):
        ...
        csvinfo.set_headings(
            "edge_length_err", "edge_length_std", "edge_angle_err", "edge_angle_std"
        )
        ...
        for top, graph in self._topologies.items():
            if self.options.count_edges_along_lattice_dirs:
                info(
                    "Computing Edge lengths along each lattice direction for %s" % (top)
                )
                n = Net(graph)
    ...
    def construct_from_prev_embedding(self, top, combo, build):
        ...
        for vert in build.sbu_vertices:
            ...
            for sbu in combo:
                ...
                if (sbu.is_metal == vertex_bu.is_metal) and (
                    len(sbu.connect_points) == len(vertex_bu.connect_points)
                ):
                    bu = deepcopy(sbu)
                    bu.vertex_id = vert
                    ...
                    for j in range(len(vertex_bu.connect_points)):
                        ...
                        bu.connect_points[j].vertex_assign = vertex_bu.connect_points[
                            j
                        ].vertex_assign
                        bu.connect_points[
                            j
                        ].bonded_cp_vertex = vertex_bu.connect_points[
                            j
                        ].bonded_cp_vertex
    ...
    def embed_sbu_combo(self, top, combo, build):
        ...
        info(
            "Setting up %s" % (self.combo_str(combo))
        ...
        if build.success and self.options.use_builds:
            ...
            debug(
                "Augmented graph consists of %i vertices and %i edges"
        ...
        if build.success:
            ...
            sym = "None"
        self.options.csv.add_data(**{"time.1": t2 - t1, "space_group.1": sym})
    ...
    def _build_structures(self):
        ...
        csvinfo = CSV(name="%s_info" % (self.options.jobname))
        csvinfo.set_headings(
            "topology", "sbus", "edge_count", "time", "space_group", "net_charge"
        )
        csvinfo.set_headings(
            "edge_length_err", "edge_length_std", "edge_angle_err", "edge_angle_std"
        )
            ...
            for top, graph in self._topologies.items():
                ...
                if self.options.count_edges_along_lattice_dirs:
                    ...
                    if build.check_net:
                        ...
                        if (
                            build.met_met_bonds
                            and run.linear_sbus_exist
                            and not run.linear_in_combo(combo)
                        ):
                            # add linear organics
                            debug(
                                "Metal-type nodes attached to metal-type nodes. "
    ...
    def _sbu_report(self):
        ...
        filename = os.path.join(
            self.options.job_dir, self.options.jobname + ".SBU_report.csv"
        )
        ...
        for file in self.options.topology_files:
            ...
            for top in db.keys():
                if top in self._topologies.keys():
                    warning(
                        "Duplicate topologies found! The topology %s" % (top)
    ...
    def _pop_unwanted_sbus(self):
        ...
        [
            remove.append(x)
            for x, sbu in enumerate(self.sbu_pool)
            if sbu.identifier not in self.options.organic_sbus and not sbu.is_metal
        ]
        # remove undesired metal SBUs
        [
            remove.append(x)
            for x, sbu in enumerate(self.sbu_pool)
            if sbu.identifier not in self.options.metal_sbus and sbu.is_metal
        ]
        remove.sort()
        ...
        for sbu_request in self.options.organic_sbus:
            if sbu_request not in [
                i.identifier for i in self.sbu_pool if not i.is_metal
            ]:
                warning(
                    "SBU id %i is not in the organic SBU database" % (int(sbu_request))
                )
        for sbu_request in self.options.metal_sbus:
            if sbu_request not in [i.identifier for i in self.sbu_pool if i.is_metal]:
                warning(
                    "SBU id %i is not in the metal SBU database" % (int(sbu_request))
                )

    def _pop_unwanted_topologies(self):
        topkeys = list(self._topologies.keys())
        [
            self._topologies.pop(k, None)
            for k in topkeys
            if k not in self.options.topologies or k in self.options.ignore_topologies
        ]
        for k in self.options.topologies:
            if k not in self._topologies.keys():
                warning(
                    "Could not find the topology %s in the current " % (k)
...
def main():
    if os.getenv("TOPCRYST_DIR") is None:
        warning(
            "No environment variable pointing to the TOPCRYST directory,"
            + " this may cause runtime problems.\n"
            + "To avoid this warning type: "
            + "export TOPCRYST_DIR=%s"
            % (os.path.join(os.path.dirname(os.path.realpath(__file__))))
        )
    os.environ["TOPCRYST_DIR"] = os.path.join(
